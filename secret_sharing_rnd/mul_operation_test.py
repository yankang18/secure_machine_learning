...
class TestCarloBeaverTriplesBasedMul(unittest.TestCase):
    ...
    def test_multiply(self):
        ...
        mul_ops[op_id]["left"] = X.shape[0]
        mul_ops[op_id]["middle_0"] = X.shape[1]
        mul_ops[op_id]["middle_1"] = Y.shape[0]
        mul_ops[op_id]["right"] = Y.shape[1]

        global_iters = 3
        print("global_iters", global_iters)

        party_a_bt_map, party_b_bt_map = create_beaver_triples(mul_ops, global_iters=global_iters)
        ...
        print("-" * 20)
        for global_index in range(global_iters):
