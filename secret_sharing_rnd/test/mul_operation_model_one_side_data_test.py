...
class TestPartyModel(unittest.TestCase):
    ...
    def test_compute_logit_with_uneven_batch(self):
        ...
        for ep in range(num_epoch):
            for batch_i in range(num_batch):
                ...
                # sends X1 to the other party
                X0, X1 = share(X_batch)

                party_a.set_batch(X0, None)
                party_b.set_batch(X1, None)

                alpha_0, beta_0 = party_a.compute_shares_for_alpha_beta_of_computing_logit(global_index)
                alpha_1, beta_1 = party_b.compute_shares_for_alpha_beta_of_computing_logit(global_index)
